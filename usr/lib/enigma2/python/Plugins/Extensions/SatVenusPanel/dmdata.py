# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/SatVenusPanel/dmdata.py
import urllib2
import HTMLParser
import cStringIO
import datetime
...
def getnew(idate = None):
    try:
        ...
        cdate = now.strftime('%d-%b-%Y')
        d1 = datetime.datetime.strptime(idate, '%d-%b-%Y')
        d2 = datetime.datetime.strptime(str(cdate), '%d-%b-%Y')
        delta = d2 - d1
        if delta.days < 32:
            return True
        return False
...
def getgz(urlStr, searchstr = None):
    data = []
    try:
    ...
    for line in raw_list:
        ...
        if searchstr:
            if searchstr == 'New':
                if line != '' and '.gz' in line:
                    nfiparts = []
                    nfiparts = line.split('.gz')
                    url = nfiparts[0] + '.gz'
                    ...
                    if getnew(idate):
                        ...
                        data.append([url,
                         ...
                         itime,
                         isize])
            elif line != '' and '.gz' in line and searchstr.lower() in line.lower():
                nfiparts = []
                nfiparts = line.split('.gz')
                url = nfiparts[0] + '.gz'
                spart = nfiparts[1].strip()
...
def getdata(urlStr, searchstr = None):
    ...
    for line in raw_list:
        ...
        if searchstr:
            if searchstr == 'New':
                if line != '' and '.nfi' in line:
                    nfiparts = []
                    nfiparts = line.split('.nfi')
                    url = nfiparts[0] + '.nfi'
                    spart = nfiparts[1].strip()
                    sizdateparts = spart.split(' ')
                    idate = sizdateparts[0]
                    ...
                    if getnew(idate):
                        ...
                        data.append([url,
                         ...
                         itime,
                         isize])
            elif line != '' and '.nfi' in line and searchstr.lower() in line.lower():
                nfiparts = []
                nfiparts = line.split('.nfi')
                url = nfiparts[0] + '.nfi'
                spart = nfiparts[1].strip()
...
def nfizip(urlStr, searchstr = None):
    data = []
    try:
        fileHandle = urllib2.urlopen(urlStr)
        html = fileHandle.read()
